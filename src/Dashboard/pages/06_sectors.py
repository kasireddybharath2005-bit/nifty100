...
sector = st.selectbox("Select Sector", sectors["sector_name"])
...
chart_df = pd.DataFrame(
    {
        ...
        "Value": [median_roe, median_de, median_asset, median_margin],
    }
)
st.markdown("---")
...
sector_percentage = (
    selected_companies / total_companies * 100 if total_companies > 0 else 0
)
...
col2.metric("Companies", selected_companies)

col3.metric("Industries", industry_count)

col4.metric("Share of Total", f"{sector_percentage:.1f}%")
st.markdown("---")

st.subheader("🏢 Industry Breakdown")
industry_df = sector_df.groupby("industry").size().reset_index(name="company_count")
fig = px.bar(
    industry_df,
    x="industry",
    y="company_count",
...
fig.update_layout(xaxis_title="Industry", yaxis_title="Number of Companies")
...
fig.update_layout(xaxis_title="Financial Metrics", yaxis_title="Median Value")
