...
import pandas as pd
from utils.db import get_companies, get_company_report, get_annual_reports
...
total_records = len(ratios) + len(pl) + len(bs) + len(cf)
...
col2.metric("Years Available", years_available)

col3.metric("Report Sections", sections)
    ...
    for _, row in reports_df.iterrows():
        ...
        with col2:
            ...
            if pd.isna(row["Annual_Report"]) or row["Annual_Report"] == "":
                ...
                st.link_button("📄 Open Annual Report", row["Annual_Report"])
st.markdown("---")
