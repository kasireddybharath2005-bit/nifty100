...
dividend = df[df["free_cash_flow"] > 0]

debtfree = df[(df["debt_to_equity"] == 0) & (df["roe_calculated"] > 12)]

turnaround = df[(df["revenue_cagr_3yr"] > 10) & (df["free_cash_flow"] > 0)]

with pd.ExcelWriter(output_path / "screener_output.xlsx") as writer:
