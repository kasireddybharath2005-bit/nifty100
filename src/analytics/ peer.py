...
for group_name, group in groups:
    ...
    for metric in metrics:
        ...
        temp["percentile"] = temp[metric].rank(pct=True) * 100

        if metric == "debt_to_equity":

            temp["percentile"] = 100 - temp["percentile"]
...
peer_percentiles.to_sql("peer_percentiles", conn, if_exists="replace", index=False)
